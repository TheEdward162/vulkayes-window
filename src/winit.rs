...
use vulkayes_core::{
	ash::{self, vk},
	instance::Instance,
	surface::{error::SurfaceError, Surface as CoreSurface},
	Vrc
};
pub use winit;
use winit::window::Window;

pub type Surface = CoreSurface<Window>;

#[cfg(all(unix, not(target_os = "android"), not(target_os = "macos")))]
pub fn create_surface(
	instance: Vrc<Instance>, window: Window,
	host_memory_allocator: vulkayes_core::memory::host::HostMemoryAllocator
) -> Result<Surface<Window>, SurfaceError> {
	...
	unsafe {
		Ok(vulkayes_core::surface::Surface::new(instance, window, surface, allocation_callbacks))
	}
}
#[cfg(all(unix, not(target_os = "android"), not(target_os = "macos")))]
pub fn required_surface_extensions() -> impl AsRef<[&'static CStr]> {
